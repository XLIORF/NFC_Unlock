...
#define HIDD_DEVICE_NAME "NFC_Authenticator"
...
void unlock_win_screen()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        esp_hidd_send_keyboard_value(hid_conn_id, 0, pass_pin + i, 1);
        vTaskDelay(5 / portTICK_PERIOD_MS);
    }
    key = 0;                                               // 取消按下的按键
    esp_hidd_send_keyboard_value(hid_conn_id, 0, &key, 1); // 用于激活输入框
    vTaskDelay(5 / portTICK_PERIOD_MS);
}
